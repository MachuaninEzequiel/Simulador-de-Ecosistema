...
# Zona regenerativa para plantas
VEGETATION_REGROWTH_TIME = 5000
vegetation_zones = [
    {"pos": [200, 200], "radius": 100, "last_regrow_time": pygame.time.get_ticks()},
    {"pos": [500, 500], "radius": 100, "last_regrow_time": pygame.time.get_ticks()}
]
...
# Función para regenerar vegetación
def regenerate_vegetation():
    for zone in vegetation_zones:
        if pygame.time.get_ticks() - zone["last_regrow_time"] >= VEGETATION_REGROWTH_TIME:
            for _ in range(2):  # Regenera 2 plantas en cada ciclo
                x = random.randint(zone["pos"][0] - zone["radius"], zone["pos"][0] + zone["radius"])
                y = random.randint(zone["pos"][1] - zone["radius"], zone["pos"][1] + zone["radius"])
                plants.append({"pos": [x, y], "color": (150, 150, 150)})
            zone["last_regrow_time"] = pygame.time.get_ticks()
...
while running:
    ...
    regenerate_vegetation()  # Regenera plantas en zonas de vegetación
